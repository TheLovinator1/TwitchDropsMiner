...
from constants import MAX_EXTRA_MINUTES
...
class BaseDrop:
    def __init__(self, campaign: DropsCampaign, data: JsonType, claimed_benefits: dict[str, datetime]) -> None:
        ...
        if "self" in data:
            ...
            (dts := [claimed_benefits[bid] for benefit in self.benefits if (bid := benefit.id) in claimed_benefits])
    ...
    def _can_earn_within(self, stamp: datetime) -> bool:
        ...
        return self._base_earn_conditions() and self.ends_at > datetime.now(UTC) and self.starts_at < stamp

    def can_earn(self, channel: Channel | None = None, ignore_channel_status: bool = False) -> bool:
        return self._base_can_earn() and self.campaign._base_can_earn(channel, ignore_channel_status)
    ...
    async def claim(self) -> bool:
        ...
        if result:
            ...
            self._twitch.print(_("status", "claimed_drop").format(drop=claim_text.replace("\n", " ")))
    ...
    async def _claim(self) -> bool:
        ...
        try:
            response = await self._twitch.gql_request(
                GQL_QUERIES["ClaimDrop"].with_variables({"input": {"dropInstanceID": self.claim_id}})
            )
        except GQLException:
        ...
        if "claimDropRewards" in data:
            if not data["claimDropRewards"]:
                return False
            if data["claimDropRewards"]["status"] in {"ELIGIBLE_FOR_ALL", "DROP_INSTANCE_ALREADY_CLAIMED"}:
                return True
        return False
...
class TimedDrop(BaseDrop):
    def __init__(self, campaign: DropsCampaign, data: JsonType, claimed_benefits: dict[str, datetime]) -> None:
        ...
        self.real_current_minutes: int = ("self" in data and data["self"]["currentMinutesWatched"]) or 0
    ...
    def total_required_minutes(self) -> int:
        return self.required_minutes + max(
            (self.campaign.timed_drops[pid].total_required_minutes for pid in self.precondition_drops),
            default=0,
        )
    ...
    def total_remaining_minutes(self) -> int:
        return self.remaining_minutes + max(
            (self.campaign.timed_drops[pid].total_remaining_minutes for pid in self.precondition_drops),
            default=0,
        )
...
class DropsCampaign:
    def __init__(self, twitch: Twitch, data: JsonType, claimed_benefits: dict[str, datetime]) -> None:
        ...
        self.timed_drops: dict[str, TimedDrop] = {
            drop_data["id"]: TimedDrop(self, drop_data, claimed_benefits) for drop_data in data["timeBasedDrops"]
        }
    ...
    def has_badge_or_emote(self) -> bool:
        return any(benefit.type.is_badge_or_emote() for drop in self.drops for benefit in drop.benefits)
    ...
    def _base_can_earn(self, channel: Channel | None = None, ignore_channel_status: bool = False) -> bool:
        return (
    ...
    def preconditions_chain(self) -> set[str]:
        return set(chain.from_iterable(drop.precondition_drops for drop in self.drops if not drop.is_claimed))

    def can_earn(self, channel: Channel | None = None, ignore_channel_status: bool = False) -> bool:
        # True if any of the containing drops can be earned
        return self._base_can_earn(channel, ignore_channel_status) and any(drop._base_can_earn() for drop in self.drops)
